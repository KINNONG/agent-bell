...
- 安装还必须在用户了解体积并明确同意后传入 `-ConfirmLargeDownload`；不要替用户预先同意大体积下载。
- 参考音频和文字只会复制到用户指定的 `InstallRoot`，不会上传到模型下载端点。
- 通过 Codex 与命令行传入的参考路径和台词会保留在当前会话及工具调用记录中；不要把它们提交到仓库或写入 Agent Bell 日志。
...
- 支持 CUDA 和 bfloat16、计算能力至少为 8.0 的 NVIDIA GPU 及可用驱动，通常为 Ampere 或更新架构
- 至少 16 GiB 系统内存和 6 GiB NVIDIA 显存
- 新安装的目标磁盘至少有 12 GiB（约 12.9 GB）可用空间

预计需要下载 `5.5–6 GB`，包括约 `2.52 GB` 的 Qwen 模型快照、CUDA PyTorch 和其他
Python 依赖；安装完成后约占用 `7.8 GB`。推荐配置为 `32 GiB` 系统内存和 `8 GiB`
NVIDIA 显存，高于安装器的 16 GiB / 6 GiB 最低要求。推荐值不是性能承诺，不同驱动、依赖
版本和同时运行的程序都会改变可用余量。
...
先向用户披露 `5.5–6 GB` 下载量、约 `7.8 GB` 安装占用和 `12 GiB`（约 `12.9 GB`）可用空间要求，
并等待用户明确确认。只有获得确认后，才在插件的 `voice-pack` 目录运行：
...
1. 在创建安装文件或开始下载前，验证大体积下载与音色授权确认，并执行只读硬件和容量检查。
...
当前 v0.1.7 安装器使用固定 revision 的官方 Hugging Face 仓库，不提供 ModelScope
切换参数。中国大陆用户访问 Hugging Face 时可能下载较慢；Qwen 官方虽然为大陆用户记录了
ModelScope 下载方式，本版仍不应擅自替换为未经校验的第三方镜像。下载条件不合适时可以继续
使用不下载模型的 Lite 模式，后续版本可以增加经过校验的镜像源支持。
...
Voice Pack 启动后模型会常驻显存，即使没有正在合成也会减少其他程序可用的 GPU 空间。
收到 UserPromptSubmit 后，后台进程会立即尝试读取真实会话名；名称已经可用时直接检查系统
内存、CPU、空闲显存和 GPU 利用率并提交预生成，新会话名称尚未写入时会在 10 秒后重试一次。
当前预生成启动前要求至少 1.5 GiB 可用系统内存、CPU 使用率不高于 75%、至少 1.5 GiB
空闲显存且 GPU 使用率不高于 70%；任一关键指标无法可靠读取时也按资源不足处理。首次检查
未通过时，仍在运行的任务会在约 15、45 和 75 秒时再检查三次，等待期间不会占用预生成活动槽。
等待 helper 最多 4 个，资源探测与提交最多同时 2 个；超过上限的并行任务会跳过本次预生成。
四次都未通过时任务完成不现场等待合成、不稍后补播，也不播放 Windows 提示音；完成事件会按
`fallback_provider` 使用 SAPI 系统语音兜底，设为 `none` 时才保持静默。
16 GiB 最低配置在生成期间仍可能发生分页，推荐 32 GiB 系统内存。

服务进程尽力使用 Windows `BelowNormal` 优先级，预生成队列最多保留两个待处理任务。这些
措施用于降低对 Codex 和前台工作的干扰，不代表本地 GPU 工作负载完全没有竞争。运行剪辑、
游戏或其他本地 AI 工具时，建议暂时使用 Lite 模式或关闭 Voice Pack 服务；权限和失败播报
仍可能触发按需合成，并在服务繁忙或超时时回退到 SAPI。
...
保留 `fallback_provider: sapi`。完成事件只读取预生成缓存，不会再等待 17 秒现场生成、稍后补播
或播放 Windows 提示音；缓存未就绪时会立即回退到 Windows 系统语音。权限和失败事件在服务
尚未启动、忙碌、超时或返回无效 WAV 时也会执行相同兜底。需要严格静默时可设为 `none`。
