...
当前 v0.1.6 安装器使用固定 revision 的官方 Hugging Face 仓库，不提供 ModelScope
...
四次都未通过时任务完成保持静默，不现场等待合成、不稍后补播，也不播放 Windows 提示音。
...
仍可回退到 Windows 系统语音。完成事件只读取预生成缓存；缓存未就绪时保持静默，不会再等待
17 秒现场生成、稍后补播或播放 Windows 提示音。
