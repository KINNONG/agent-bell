...
当前 v0.1.5 安装器使用固定 revision 的官方 Hugging Face 仓库，不提供 ModelScope
...
当前预生成要求至少 2 GiB 可用系统内存、CPU 使用率不高于 75%、至少 1.5 GiB 空闲显存且
GPU 使用率不高于 70%；任一关键指标无法可靠读取时也按资源不足处理。本次预生成会正常跳过，
任务完成时保持静默，不现场等待合成、不稍后补播，也不播放 Windows 提示音。
