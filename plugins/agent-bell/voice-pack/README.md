...
当前 v0.1.4 安装器使用固定 revision 的官方 Hugging Face 仓库，不提供 ModelScope
...
后台完成播报不会一收到 Hook 就立即抢占资源：它默认先等待 10 秒，再检查系统内存、CPU、
空闲显存和 GPU 利用率。当前预生成要求至少 2 GiB 可用系统内存、CPU 使用率不高于 75%、
至少 1.5 GiB 空闲显存且 GPU 使用率不高于 70%；任一关键指标无法可靠读取时也按资源不足
处理。本次预生成会正常跳过，任务完成时只响一次 Windows 提示音，不现场等待合成，也不会
稍后补播。
